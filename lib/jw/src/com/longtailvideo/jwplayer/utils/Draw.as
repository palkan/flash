package com.longtailvideo.jwplayer.utils {
	...
	public class Draw {
		...
		/**
		 * Draw a gradient rectangle on stage.
		 *
		 * @param tgt	Displayobject to add the rectangle to.
		 * @param cols	Colors of the gradient.
		 * @param alphas Aplhas of the gradient.
		 ...
		 * @return		A reference to the newly drawn rectangle.
		 **/
		public static function gradientRect(tgt:Sprite, cols:Array, alphas:Array,ratios:Array, wid:Number, hei:Number, xps:Number = 0, yps:Number = 0):Sprite {
			var rct:Sprite = new Sprite();
			var matrix:Matrix = new Matrix();
			matrix.createGradientBox(wid, hei, (Math.PI/180)*90, 0, 00);
			rct.x = xps;
			rct.y = yps;
			rct.graphics.beginGradientFill(GradientType.LINEAR,cols,alphas,ratios, matrix);
			rct.graphics.drawRect(0, 0, wid, hei);
			tgt.addChild(rct);
			return rct;
		}
